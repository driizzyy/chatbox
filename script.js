// ChatBox v2.0 - Real-Time Chat Application with Authentication
// Version: 2.0 - Enhanced with Advanced Features and User Accounts
...
class ChatBox {
    ...
    async initElements() {
        ...
        this.lastActive = document.getElementById('lastActive');
        this.closeProfile = document.getElementById('closeProfile');
        this.cancelProfileChanges = document.getElementById('cancelProfileChanges');
        this.saveProfileChanges = document.getElementById('saveProfileChanges');
    ...
    bindEvents() {
        ...
        if (this.usernameInput) {
            this.usernameInput.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') {
                    // Determine if we should sign in or register based on form state
                    if (this.registerFields && this.registerFields.style.display !== 'none') {
                        this.handleRegister();
                    } else {
                        this.handleSignIn();
                    }
        ...
        if (this.registerBtn) {
            this.registerBtn.addEventListener('click', () => {
                ...
                if (this.registerFields && (this.registerFields.style.display === 'none' || this.registerFields.style.display === '')) {
                    // Show register fields
                    this.showRegisterFields();
                } else {
                    // Fields are visible, handle registration
                    this.handleRegister();
                }
            });
        }
        ...
        if (this.userAvatar) {
            this.userAvatar.addEventListener('click', () => this.openProfileModal());
        }
        ...
        if (this.profileBio) {
            this.profileBio.addEventListener('input', () => this.updateBioCharCount());
        }
        
        if (this.changePasswordBtn) {
            this.changePasswordBtn.addEventListener('click', () => this.handlePasswordChange());
        }
        ...
        if (this.cancelProfileChanges) {
            this.cancelProfileChanges.addEventListener('click', () => this.closeProfileModal());
        }
        
        if (this.saveProfileChanges) {
            this.saveProfileChanges.addEventListener('click', () => this.saveProfile());
        }

        // Welcome screen events (legacy guest mode)
        if (this.joinBtn) {
    ...
    showWelcomeScreen() {
        if (this.welcomeScreen) {
            ...
            // Add desktop optimization classes for fullscreen mode
            this.welcomeScreen.classList.add('desktop-optimized');
            document.body.classList.add('welcome-mode');
            if (this.appContainer) {
                this.appContainer.classList.add('welcome-mode');
            }
        }
    ...
    showChatInterface() {
        if (this.welcomeScreen) {
            ...
            // Remove desktop optimization classes when switching to chat
            this.welcomeScreen.classList.remove('desktop-optimized');
            document.body.classList.remove('welcome-mode');
            if (this.appContainer) {
                this.appContainer.classList.remove('welcome-mode');
            }
        }
        if (this.chatInterface) {
    ...
    showRegisterFields() {
        if (this.registerFields) {
            ...
            if (this.registerBtn) {
                this.registerBtn.innerHTML = '<span class="btn-text"><i class="fas fa-user-plus"></i> Create Account</span>';
            }
        }
    }
    ...
    hideRegisterFields() {
        if (this.registerFields) {
            ...
            // Reset button text
            if (this.registerBtn) {
                this.registerBtn.innerHTML = '<span class="btn-text"><i class="fas fa-user-plus"></i> Register</span>';
            }
        }
    }
    ...
    handleSignIn() {
        ...
        this.signinBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Signing in...';
    ...
    async performLogin(username, password) {
        try {
            ...
            if (data.success) {
                ...
                localStorage.setItem('chatbox-auth-token', data.token);
                localStorage.setItem('chatbox-user-profile', JSON.stringify(data.profile));
            ...
            this.signinBtn.innerHTML = '<span class="btn-text"><i class="fas fa-sign-in-alt"></i> Sign In</span>';
        }
    }
    ...
    async handleRegister() {
        ...
        if (username.length < 3 || username.length > 20) {
            this.showToast('Username must be 3-20 characters', 'error');
            return;
        }
    ...
    handleGuestLogin() {
        ...
        if (!username || username.length < 1 || username.length > 20) {
            this.showToast('Please enter a valid display name (1-20 characters)', 'error');
            return;
        }
        ...
        this.showToast('Joining as guest...', 'info');
        this.connectToServer();
    }
    
    // Legacy methods (keeping for compatibility)
    showAuthForm(type) {
        // This method is no longer needed with the unified form
        console.log('Auth form type:', type);
    }
    
    async handleLogin() {
        // Redirect to new sign in method
        return this.handleSignIn();
    }
    
    async checkAuthenticationStatus() {
        const token = localStorage.getItem('chatbox-auth-token');
        const profileData = localStorage.getItem('chatbox-user-profile');
        ...
        try {
            ...
            if (data.success) {
                ...
                // Clear invalid token
                localStorage.removeItem('chatbox-auth-token');
                localStorage.removeItem('chatbox-user-profile');
                return false;
            }
        } catch (error) {
    ...
    updateUserAvatar() {
        ...
        if (this.isAuthenticated && this.userProfile && this.userProfile.avatar) {
            this.userAvatar.innerHTML = `<img src="${this.userProfile.avatar}" alt="Profile" class="avatar-image">`;
        } else if (this.username) {
            this.userAvatar.innerHTML = this.username.charAt(0).toUpperCase();
        } else {
    ...
    openProfileModal() {
        if (!this.isAuthenticated) {
            this.showToast('Profile settings are only available for registered users', 'info');
            return;
        }
    ...
    loadProfileData() {
        ...
        if (this.profileUsername) this.profileUsername.value = this.userProfile.username;
        if (this.profileDisplayName) this.profileDisplayName.value = this.userProfile.displayName || this.userProfile.username;
        if (this.profileBio) this.profileBio.value = this.userProfile.bio || '';
        if (this.profileAvatarPreview) {
            this.profileAvatarPreview.src = this.userProfile.avatar || '/default-avatar.png';
        }
        
        // Update stats
        if (this.memberSince && this.userProfile.joinedAt) {
            this.memberSince.textContent = new Date(this.userProfile.joinedAt).toLocaleDateString();
        }
        if (this.lastActive && this.userProfile.lastActive) {
            this.lastActive.textContent = this.getTimeAgo(new Date(this.userProfile.lastActive).getTime());
        }
    ...
    updateBioCharCount() {
        if (this.profileBio && this.bioCharCount) {
            const length = this.profileBio.value.length;
            this.bioCharCount.textContent = length.toString();
            
            if (length > 150) {
                this.bioCharCount.style.color = '#ef4444';
            } else if (length > 120) {
                this.bioCharCount.style.color = '#f59e0b';
            } else {
                this.bioCharCount.style.color = '';
            }
        }
    }
    ...
    async handleAvatarUpload(event) {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.profileAvatarPreview.src = data.avatar;
                this.updateUserAvatar();
                this.showToast('Avatar updated successfully!', 'success');
                
                // Update stored profile
                localStorage.setItem('chatbox-user-profile', JSON.stringify(this.userProfile));
            } else {
    ...
    async removeAvatar() {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.profileAvatarPreview.src = '/default-avatar.png';
                this.updateUserAvatar();
    ...
    async saveProfile() {
        const displayName = this.profileDisplayName?.value.trim();
        const bio = this.profileBio?.value.trim();
        ...
        if (bio && bio.length > 150) {
            this.showToast('Bio must be 150 characters or less', 'error');
            return;
        }
    ...
    bindSocketEvents() {
        this.socket.on('connect', () => {
            ...
            console.log('Joining room:', this.currentRoom, 'as user:', this.username); // Debug log
        ...
        this.socket.on('users_update', (users) => {
            console.log('Received users_update:', users); // Debug log
    ...
    handleGlobalClick(e) {
        ...
        // Hide register fields if clicking outside the auth container
        if (this.registerFields && this.registerFields.style.display === 'block') {
            const authContainer = e.target.closest('.auth-container');
            if (!authContainer) {
                this.hideRegisterFields();
            }
        }
    }
